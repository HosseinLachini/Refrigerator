...
#define COMPRESSOR_ON_6_HOUR                (6*60)      // 6 hour
#define COMPRESSOR_ON_24_HOUR               (24*60+40)     // 24 hour
...
#define COMPRESSOR_SUPER_ON_TIME            COMPRESSOR_ON_24_HOUR
...
namespace Compressor {
    ...
    {
        return (Sensors::readAmbiance() > (settings.temperature + 20));
    }

    bool cut_out_check() // off compressor
    {
        if(settings.state & REFRIGERATOR_STATE_SUPER)
            return ((Sensors::readAmbiance() <= (-31*10)) || (readTimer() >= COMPRESSOR_SUPER_ON_TIME));
        return (Sensors::readAmbiance() <= (settings.temperature - (1*10)));
    }

    void setCompressor(uint8_t val)
    {
        if(((hardware::getCompressor() == 0) && val && ((settings.state & (REFRIGERATOR_STATE_PLUGIN_DEFROST | REFRIGERATOR_STATE_DEFROST)) == 0))
            || (hardware::getCompressor() && (val == 0))
            )
        {
            if(val == 0)
            {
                work_timer_U16_ += readTimer();
                switch(Door::ReadCounter())
                {
                    case 0 :
                        if(readWorkTimer() >= (30*60))
                        {
...
void Compressor::doIdle()
{
    ...
    if((settings.state & (REFRIGERATOR_STATE_ERROR_AMBI | REFRIGERATOR_STATE_ERROR_EVAP)) == REFRIGERATOR_STATE_ERROR_AMBI)
    {
        ...
        if((hardware::getCompressor() == 0)
            && ((readTimer() >= (30)) || (state == COMPRESSOR_STATE_START))
        )
        {
            setCompressor(1);
            return;
    ...
    if(cut_in_check()
     || ((settings.state & REFRIGERATOR_STATE_SUPER) && (Sensors::readAmbiance() > -290))
     )
    {
        setCompressor(1);
        return;
...
{
    return (Time::diffU16(timer_U16_, Time::getMinutesU16()));
}
